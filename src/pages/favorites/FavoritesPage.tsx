      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <h3 className="mt-3 font-serif text-2xl text-stone-900">收藏列表</h3>
          </div>
          <button
        ...
        {!error && filteredItems.length === 0 ? <p className="mt-6 text-sm text-stone-500">这个分类里还没有留下来的句子。</p> : null}
        ...
        <div className={viewMode === "gallery" ? "mt-6 grid gap-4" : "mt-6 space-y-3"}>
          {filteredItems.map((item) => (
            <article
              ...
              className={
                viewMode === "gallery"
                  ? "rounded-[1.5rem] border border-stone-200/80 bg-[#f8f4eb] p-5"
                  : "flex items-center justify-between rounded-[1.25rem] border border-stone-200/80 bg-[#f8f4eb] p-4"
              }
            >
