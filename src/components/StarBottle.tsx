    ...
    >
      <div className={`relative h-40 w-32 transition-all duration-500 ${getShapeStyles()}`}>
        ...
        >
          ...
          <div className="pointer-events-none absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 opacity-30">
            ...
            <div className={`mx-auto mt-0.5 h-1.5 w-3 rounded-full border-b-2 border-current ${isDarkMode ? "text-white" : "text-black"}`} />
          </div>
        ...
        <div
          className={`absolute -top-2 left-1/2 z-10 h-6 w-12 -translate-x-1/2 rounded-t-xl shadow-md ${isDarkMode ? "bg-stone-700" : "bg-stone-400"}`}
        >
      ...
      <div className="mt-5 text-center">
        <div className="mb-1 flex items-center justify-center gap-1">
          <Star className={isDarkMode ? "text-stone-500" : "text-stone-400"} fill="currentColor" size={8} />
          <p className={`text-[10px] font-bold uppercase tracking-[0.2em] ${isDarkMode ? "text-stone-300" : "text-stone-600"}`}>{label}</p>
          <Star className={isDarkMode ? "text-stone-500" : "text-stone-400"} fill="currentColor" size={8} />
        </div>
      </div>
    </motion.div>
